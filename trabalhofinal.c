...
typedef struct Veiculo {
    char dono[MAX_STRING];
    char modelo[MAX_STRING];
    char chassi[MAX_STRING];
...
void adicionarVeiculo(Veiculo** lista) {
    Veiculo* veiculo_novo = criarVeiculo();
    if (veiculo_novo == NULL) {
...
void listarVeiculos(Veiculo* lista, int opcao) {
    ...
    switch (opcao) {
        case 1:
            ...
            break;
        case 2:
            printf("Placas que começam com J e terminam com 0, 2, 4 ou 7 e seus respectivos donos:\n");
            while (lista != NULL) {
